...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchImagesAndUpdateCart = async () => {
      try {
        const response = await axios.get("http://52.55.8.58:8080/api/products");
          ...
          updatedCartItems.map(async (item) => {
            try {
              const res = await axios.get(
                `http://52.55.8.58:8080/api/product/${item.id}/image`,
                { responseType: "blob" }
              );
  ...
  const handleCheckout = async () => {
    try {
      ...
      const orderRes = await api.post("/customer/orders/checkout", {
      ...
      if (!internalOrderId) {
        alert("Order created but order id not found. Check backend checkout response.");
        return;
      }

      const paymentRes = await api.post(
        `/customer/payments/razorpay/create/${internalOrderId}`
      );
      ...
      const options = {
        ...
        order_id: paymentRes.data.orderId || paymentRes.data.razorpayOrderId,

        handler: async function (response) {
          try {
            await api.post("/customer/payments/razorpay/verify", {
      ...
      <div className="container-fluid px-4">
        <div className="cart-theme-card card shadow border-0 rounded-4">
          <div className="card-body p-4">
            <h2 className="fw-bold mb-4 border-bottom pb-3">Shopping Bag</h2>
